...
public class Solitaire {
	...
	void jokerA() {
		...
		do {
			if (current.cardValue == 27){
				...
				temp.cardValue = current.next.cardValue;
				current.next.cardValue = current.cardValue;
				current.cardValue = temp.cardValue;
	...
	void countCut() {
		CardNode rear = deckRear;
		CardNode current = deckRear.next;
		int rearValue = rear.cardValue;

		CardNode tmp = new CardNode();
		tmp.cardValue = current.cardValue;
		current.cardValue = rear.cardValue;
		rear.cardValue = tmp.cardValue;

		for (int i=1; i<rearValue; i++){
			CardNode temp = new CardNode();
			temp.cardValue = current.next.cardValue;
			current.next.cardValue = current.cardValue;
			current.cardValue = temp.cardValue;
